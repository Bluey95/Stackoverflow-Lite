...
class Answer(object):
    ...
    def update(self, answer_id):
        ...
        if res:
            is_accepted = False
            question_id = res[3]
            cur.execute("UPDATE answers SET body = %s, answered_by = %s, user_id = %s, question_id = %s, \
                is_accepted = %s, votes = %s WHERE id = %s;", \
    ...
    def upvote(self, answer_id):
        ...
        if res:
            ...
            cur.execute("UPDATE answers SET body = %s, answered_by = %s, user_id = %s, question_id = %s, \
                 votes = %s WHERE id = %s;", \
                (self.body, g.username, g.userid, question_id, upvote, answer_id)
            )
            item = self.fetch_answer(answer_id)
            self.save()
    ...
    def downvote(self, answer_id):
        ...
        if res:
            if res[6] > 0:
                ...
                cur.execute("UPDATE answers SET body = %s, answered_by = %s, user_id = %s, question_id = %s, \
                    votes = %s WHERE id = %s;", \
                    (self.body, g.username, g.userid, question_id, downvote, answer_id)
                )
                item = self.fetch_answer(answer_id)
                self.save()
